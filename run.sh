...
export MESSAGE="[$WERCKER_GIT_REPOSITORY:$WERCKER_GIT_BRANCH] <$ACTION_URL|${ACTION} ${WERCKER_GIT_COMMIT:0:8}> by $WERCKER_STARTED_BY $WERCKER_RESULT"
export FALLBACK="[$WERCKER_GIT_REPOSITORY:$WERCKER_GIT_BRANCH] ${ACTION} ${WERCKER_GIT_COMMIT:0:8} by $WERCKER_STARTED_BY $WERCKER_RESULT"
export COLOR="good"
